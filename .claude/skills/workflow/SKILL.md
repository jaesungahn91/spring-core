---
name: workflow
description: 이슈 생성부터 PR 생성까지 전체 흐름을 오케스트레이션한다. (issue-create → 브랜치 생성 → pr-quality-check → pr-document → pr-create)
...
1. `pr-quality-check` 스킬 호출
   - 실패 시: 실패 내용 출력 후 중단. 수정 후 다시 `/workflow continue` 실행 안내
   - 성공 시: 다음 단계 진행
2. `pr-document` 스킬 호출 → 이슈 댓글 게시 + PR 본문용 작업 노트 수집
3. `pr-create` 스킬 호출 (작업 노트를 PR 본문에 포함)
4. PR URL 출력
...
- pr-quality-check는 변경된 모듈만 대상으로 실행 (git diff로 파악)
